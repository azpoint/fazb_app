...
export async function sendVerificationEmail(recipientEmail) {
    ...
    try {
        ...
        if (!checkCode) {
            ...
            console.log("__EMAIL_SENT!!!");

            return { success: true };
        } else {
            const timePassed = Math.floor((Date.now() - new Date(checkCode.createdAt).getTime()) / 60000);

            console.log("__timepassed", timePassed);

            if (checkCode && timePassed > 5) {
